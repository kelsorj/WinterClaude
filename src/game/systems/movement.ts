...
function blocked(state: GameState, q: Vec2): boolean {
...
export function movePlayer(state: GameState, intent: Vec2, dt: number): void {
  ...
  const stuck = blocked(state, p.pos);
  let next = v(clampX(p.pos.x + step.x), p.pos.z);
  if (!stuck && blocked(state, next)) next = v(p.pos.x, p.pos.z);
  const withZ = v(next.x, clampZ(p.pos.z + step.z));
  if (stuck || !blocked(state, withZ)) next = withZ;
  p.pos = next;
}
